...
CNN::CNN(Size size_input, Size size_convolution, Size size_pool, Size size_output, int number_feature_maps) {
...
void CNN::train_data(DataPreparation *data, int epochs) {
...
void CNN::back_propagation(Mat input_train, Mat target_train) {
    // feed-forward
    ...
    // backward-pass
    Mat delta = cost_derivative(Sigmoid::Compute(output_layer),target_train).mul(Sigmoid::ComputePrime(output_layer));
    
    biases_of_fully_conn_maps = delta;
    weights_of_fully_conn_maps = delta * pool_vector.t();
}



void CNN::convolutional_layer(Mat local_field_of_imsource) {
...
void CNN::pooling_layer(Mat conv_layer_by_index, int index) {
...
void CNN::fully_connected_layer(Mat pool_layer) {
...
Mat CNN::cost_derivative(Mat output_activations, Mat y_target) {
