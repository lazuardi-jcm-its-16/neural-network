    ...
    for(int i=0; i<epochs; ++i) {
        for(int j=0; j<n/mini_batch_size; ++j) {
            for(int z=0; z<mini_batch_size; ++z) {
                mini_batch_train[z] = nn.data->train_vectors[j*mini_batch_size+z];
                mini_batch_train_label[z] = nn.data->train_labels[j*mini_batch_size+z];
            }
...
void NN2::update_mini_batch(Mat * mini_batch_train, Mat * mini_batch_train_label, int mini_batch_size, double learning_rate) {
